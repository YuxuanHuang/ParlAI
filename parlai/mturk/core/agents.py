...
class MTurkManager():
    ...
    def get_agent_work_status(self, conversation_id, agent_id):
        params = {
            'method_name': 'get_agent_work_status',
            'task_group_id': self.task_group_id,
            'conversation_id': conversation_id,
            'agent_id': agent_id,
        }
...
class MTurkAgent(Agent):
    ...
    def approve_work(self):
        if self.manager.get_agent_work_status(conversation_id=self.conversation_id, agent_id=self.id) == 'done':
    ...
    def wait_for_hit_completion(self):
        while self.manager.get_agent_work_status(conversation_id=self.conversation_id, agent_id=self.id) != 'done':
            if debug:
