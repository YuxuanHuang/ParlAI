...
def set_agent_work_status(event, context):
    if event['method'] == 'POST':
        """
        Expects <task_group_id>, <conversation_id>, <agent_id>, <assignmentId>, <hitId>, <workerId>, <is_sandbox>, <work_status> as POST body parameters
        """
        params = event['body']
        ...
        data_model.set_agent_work_status(
...
def get_agent_work_status(event, context):
    if event['method'] == 'GET':
        """
        Expects <task_group_id>, <conversation_id>, <agent_id> as query parameters.
        """
        try:
            task_group_id = event['query']['task_group_id']
            ...
            work_status = data_model.get_agent_work_status(
                db_session = db_session, 
                task_group_id = task_group_id, 
                conversation_id = conversation_id, 
                agent_id = agent_id
            ...
            if work_status == None:
                return json.dumps({'work_status': 'in_progress'})
            else:
                return json.dumps({'work_status': work_status})
        except KeyError:
            raise Exception('400')
